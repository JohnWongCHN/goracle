# 基于 `Go` 语言的 `Zabbix agent2` 监控 `Oracle` 数据库外部插件

自 `Zabbix 6.0 LTS` 发布以来，`Zabbix Agent2` 终于开始支持外部插件（[External Plugin](https://www.zabbix.com/documentation/guidelines/en/plugins#external-plugins)），进一步提升了 `Zabbix` 扩展能力。
此插件基于官方的 `Oracle` 插件模块修改而来，官方插件使用了 `Oracle 12c` 及更新版本开始支持的 `JSON` 语法，为了
兼容`11g`这些没有`JSON`特性的版本而改用传统的 `SQL` 语法，

## 编译要求

* Go >= 1.5 (required only to build from source)
...
* Configure `goracle` plugin.

> plugin default directory commonly is `/etc/zabbix/zabbix_agent2.d/plugins.d/` or
> you can check `zabbix_agent2.conf` file for real plugin conf directory

* Restart zabbix agent2.

## 编译说明

see [HOW-TO](docs/build.md).

## 配置

The `goracle` plugin conf file is used to configure plugins, you can check [goracle](conf/goracle.conf) conf example for all supported parameter.

**Plugins.Goracle.System.Path** - Path to external plugin executable.  
*Mandatory:* yes.  
*Default value:* none.  
